...
public class Book {
	...
	public static Boolean isTitleWord(String word) {
		String[] article ={"a" , "an", "the","and", "but", "for", "nor", "or", "so", "yet","at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
		for (int i =0 ; i<article.length;i++){
			if(word.equalsIgnoreCase(article[i])){
				return false;
			}
		}
	...
	public void setAuthor(String author) {
		...
		if(author.isEmpty()){
			...
			for(int i = 0 ; i<arrAuthor.length;i++){
				trm += capitalize(arrAuthor[i])+" ";
			}
			this.author = trm.trim();
	...
	public void setTitle(String title) {
		...
		if(!title.isEmpty()){
			...
			this.title = capitalize(arrtitle[0])+" ";
			for(int i=1; i<arrtitle.length;i++){
				if(isTitleWord(arrtitle[i])){
					this.title+=capitalize(arrtitle[i])+" ";
				}else{
					this.title+=arrtitle[i].toLowerCase()+" ";
				}
			}
		}
